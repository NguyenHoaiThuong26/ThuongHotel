import React, { useState, useEffect } from "react"
import { API_BASE_URL } from "../../configuration/configuration"
import { Eye, EyeOff, Building2, Mail, Lock, Loader2, QrCode, ArrowLeft, RefreshCw } from "lucide-react"
import { Link, useNavigate } from 'react-router-dom'
import { hasRoleSync } from "../../utils/auth"
import QRCode from "react-qr-code"

export default function LoginPage() {
  const navigate = useNavigate()
  const [email, setEmail] = useState("")
  const [password, setPassword] = useState("")
  const [showPassword, setShowPassword] = useState(false)
  const [isLoading, setIsLoading] = useState(false)
  const [errors, setErrors] = useState<{ email?: string; password?: string }>({})
  const [isQRLogin, setIsQRLogin] = useState(false)

  // QR Code Timer Logic
  const [countdown, setCountdown] = useState(120) // 2 minutes

  // Auto-redirect if already logged in
  useEffect(() => {
    const checkLogin = async () => {
      const token = localStorage.getItem("token")
      if (token) {
        try {
          const response = await fetch(`${API_BASE_URL}/users/myInfo`, {
            headers: { Authorization: `Bearer ${token}` }
          })
          if (response.ok) {
            const data = await response.json()
            const roles = data.result?.roles || []
            if (hasRoleSync(roles, "ADMIN") || hasRoleSync(roles, "STAFF") || hasRoleSync(roles, "RECEPTIONIST")) {
              navigate("/admin", { replace: true })
            } else {
              navigate("/", { replace: true })
            }
          } else {
            localStorage.removeItem("token")
          }
        } catch (error) {
          console.error("Error checking login status", error)
        }
      }
    }
    checkLogin()
  }, [navigate])

  // Timer Effect
  useEffect(() => {
    let timer: ReturnType<typeof setTimeout>
    if (isQRLogin && countdown > 0) {
      timer = setInterval(() => {
        setCountdown((prev) => prev - 1)
      }, 1000)
    }
    return () => {
      if (timer) clearInterval(timer)
    }
  }, [isQRLogin, countdown])

  // Reset timer when opening QR view
  useEffect(() => {
    if (isQRLogin) {
      setCountdown(120)
    }
  }, [isQRLogin])

  const formatTime = (seconds: number) => {
    const m = Math.floor(seconds / 60)
    const s = seconds % 60
    return `${m}:${s < 10 ? "0" : ""}${s}`
  }

  const handleReloadQR = () => {
    setCountdown(120)
  }

  const validateForm = () => {
    const newErrors: { email?: string; password?: string } = {}

    if (!email.trim()) newErrors.email = "Email hoặc tên đăng nhập là bắt buộc"
    if (!password.trim()) newErrors.password = "Mật khẩu là bắt buộc"

    setErrors(newErrors)
    return Object.keys(newErrors).length === 0
  }

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!validateForm()) return

    setIsLoading(true)
    try {
      const response = await fetch(`${API_BASE_URL}/auth/token`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ username: email, password }),
      })

      if (response.ok) {
        const data = await response.json()
        console.log("Đăng nhập thành công:", data)
        if (data.result && data.result.token) {
          localStorage.setItem("token", data.result.token)

          // Check role for redirect
          try {
            const userResponse = await fetch(`${API_BASE_URL}/users/myInfo`, {
              headers: { Authorization: `Bearer ${data.result.token}` }
            })
            if (userResponse.ok) {
              const userData = await userResponse.json()
              const roles = userData.result?.roles || []
              if (hasRoleSync(roles, "ADMIN") || hasRoleSync(roles, "STAFF") || hasRoleSync(roles, "RECEPTIONIST")) {
                window.location.href = "/admin"
                return
              } else {
                window.location.href = "/"
                return
              }
            }
          } catch (e) {
            console.error("Error fetching user info", e)
          }

          navigate("/")
          window.location.reload()
        }
      } else {
        setErrors({ email: "Thông tin đăng nhập không hợp lệ" })
      }
    } catch (error) {
      console.error("Lỗi đăng nhập:", error)
      setErrors({ email: "Đã xảy ra lỗi. Vui lòng thử lại." })
    } finally {
      setIsLoading(false)
    }
  }

  const handleGoogleLogin = () => {
    // Redirect to backend OAuth2 endpoint
    window.location.href = `${API_BASE_URL}/oauth2/authorization/google`;
  };

  return (
    <div className="min-h-screen flex">
      {/* Bên trái */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden 
                  bg-gradient-to-br from-teal-400 to-blue-500">
        <div
          className="absolute inset-0 bg-cover bg-center opacity-40"
          style={{
            backgroundImage:
              "url(/placeholder.svg?height=1080&width=960&query=luxury%20hotel%20interior)",
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-r from-teal-400/80 to-blue-500/60" />
        <div className="relative z-10 flex flex-col justify-center items-center w-full px-8 text-white">
          <div className="text-center max-w-md">
            <Building2 className="w-16 h-16 mx-auto mb-6 text-white" />
            <h1 className="text-4xl font-bold mb-4">Chào mừng đến với Thuong Hotel</h1>
            <p className="text-white text-lg leading-relaxed">
              Trải nghiệm dịch vụ khách sạn đẳng cấp thế giới. Quản lý đặt phòng của bạn dễ dàng.
            </p>
          </div>
        </div>
      </div>

      {/* Bên phải */}
      <div className="w-full lg:w-1/2 flex items-center justify-center px-4 sm:px-6 lg:px-8 bg-white pt-10">
        <div className="w-full max-w-md">
          {/* Logo trên mobile */}
          <div className="lg:hidden flex justify-center mb-8">
            <Building2 className="w-12 h-12 text-teal-600" />
          </div>

          {isQRLogin ? (
            // QR Code View
            <div className="text-center animate-in fade-in zoom-in duration-300">
              <button
                onClick={() => setIsQRLogin(false)}
                className="mb-6 flex items-center text-slate-600 hover:text-teal-600 transition"
              >
                <ArrowLeft className="w-5 h-5 mr-2" /> Quay lại
              </button>

              <h2 className="text-3xl font-bold text-slate-900 mb-2">Đăng nhập bằng QR</h2>
              <p className="text-slate-600 mb-8">Quét mã QR bằng ứng dụng di động để đăng nhập nhanh</p>

              <div className="bg-white p-6 rounded-2xl shadow-lg border border-slate-100 inline-block relative overflow-hidden">
                <div className={`relative transition-all duration-300 ${countdown === 0 ? "blur-sm opacity-50" : ""}`}>
                  <QRCode
                    value="https://example.com/login-qr-mock"
                    size={220}
                    level="H"
                    fgColor="#0d9488"
                  />
                  <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="bg-white p-1 rounded-full">
                      <Building2 className="w-8 h-8 text-teal-600" />
                    </div>
                  </div>
                </div>

                {countdown === 0 && (
                  <div className="absolute inset-0 flex items-center justify-center bg-white/60 z-10">
                    <button
                      onClick={handleReloadQR}
                      className="bg-teal-600 text-white p-3 rounded-full shadow-lg hover:bg-teal-700 transition animate-in zoom-in duration-300"
                      title="Làm mới mã QR"
                    >
                      <RefreshCw className="w-6 h-6" />
                    </button>
                  </div>
                )}
              </div>

              <p className="text-slate-500 text-sm mt-8">
                {countdown > 0 ? (
                  <>Mã QR sẽ hết hạn sau <span className="text-teal-600 font-semibold">{formatTime(countdown)}</span></>
                ) : (
                  <span className="text-red-500 font-semibold">Mã QR đã hết hạn</span>
                )}
              </p>
            </div>
          ) : (
            // Normal Login View
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
              <div className="mb-8 text-left">
                <h2 className="text-3xl font-bold text-slate-900">Đăng nhập</h2>
                <p className="text-slate-600 mt-2">Nhập thông tin của bạn để truy cập tài khoản</p>
              </div>

              <form onSubmit={handleLogin} className="space-y-5">
                {/* Email */}
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-slate-700 mb-2 text-left">
                    Email hoặc tên đăng nhập
                  </label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                    <input
                      id="email"
                      type="text"
                      value={email}
                      onChange={(e) => {
                        setEmail(e.target.value)
                        if (errors.email) setErrors({ ...errors, email: undefined })
                      }}
                      placeholder="your@email.com"
                      className={`w-full pl-10 pr-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 transition ${errors.email ? "border-red-500 bg-red-50" : "border-slate-300 bg-slate-50 hover:border-slate-400"
                        }`}
                    />
                  </div>
                  {errors.email && <p className="text-red-600 text-sm mt-1">{errors.email}</p>}
                </div>

                {/* Mật khẩu */}
                <div>
                  <label htmlFor="password" className="block text-sm font-medium text-slate-700 mb-2 text-left">
                    Mật khẩu
                  </label>
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400" />
                    <input
                      id="password"
                      type={showPassword ? "text" : "password"}
                      value={password}
                      onChange={(e) => {
                        setPassword(e.target.value)
                        if (errors.password) setErrors({ ...errors, password: undefined })
                      }}
                      placeholder="••••••••"
                      className={`w-full pl-10 pr-12 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 transition ${errors.password ? "border-red-500 bg-red-50" : "border-slate-300 bg-slate-50 hover:border-slate-400"
                        }`}
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword(!showPassword)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition"
                    >
                      {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                    </button>
                  </div>
                  {errors.password && <p className="text-red-600 text-sm mt-1">{errors.password}</p>}
                </div>

                {/* Nút đăng nhập */}
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-teal-500 to-cyan-500 hover:brightness-105 disabled:bg-teal-400 text-white font-semibold py-2.5 rounded-lg transition flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
                      <Loader2 className="w-5 h-5 animate-spin" />
                      Đang đăng nhập...
                    </>
                  ) : (
                    "Đăng nhập"
                  )}
                </button>

                {/* Divider */}
                <div className="relative my-6">
                  <div className="absolute inset-0 flex items-center">
                    <div className="w-full border-t border-slate-300" />
                  </div>
                  <div className="relative flex justify-center text-sm">
                    <span className="px-2 bg-white text-slate-500">Hoặc tiếp tục với</span>
                  </div>
                </div>

                {/* Đăng nhập Google */}
                <button
                  type="button"
                  onClick={handleGoogleLogin}
                  className="w-full border-2 border-slate-300 hover:border-slate-400 hover:bg-slate-50 text-slate-700 font-semibold py-2.5 rounded-lg transition flex items-center justify-center gap-3"
                >
                  <svg className="w-5 h-5" viewBox="0 0 24 24">
                    <path fill="currentColor" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
                    <path fill="currentColor" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
                    <path fill="currentColor" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
                    <path fill="currentColor" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
                  </svg>
                  Đăng nhập với Google
                </button>

                <button
                  type="button"
                  onClick={() => setIsQRLogin(true)}
                  className="w-full border-2 border-slate-300 hover:border-slate-400 hover:bg-slate-50 text-slate-700 font-semibold py-2.5 rounded-lg transition flex items-center justify-center gap-3"
                >
                  <QrCode className="w-5 h-5" />
                  Đăng nhập bằng QR
                </button>
              </form>

              {/* Chuyển sang đăng ký */}
              <div className="mt-8 text-center">
                <p className="text-slate-600">
                  Chưa có tài khoản?{" "}
                  <Link to="/register" className="text-teal-600 hover:text-teal-700 font-semibold transition">
                    Đăng ký ngay
                  </Link>
                </p>
              </div>

              {/* Footer */}
              <div className="mt-8 pt-6 border-t border-slate-200 text-center text-xs text-slate-500">
                <p>© 2025 Thuong Hotel. Bảo lưu mọi quyền.</p>
              </div>
            </div>
          )}
        </div>
      </div>
    </div>
  )
}
